-- ============================================================
-- OutLiner(完全自動同期アウトライナーメモアプリ): Supabaseスキーマ
...
-- Authentication > Providers > Email が有効になっていることを確認してください
-- (Supabaseプロジェクトの既定で有効です)。
-- PC⇄iPad(複数端末)間で同じデータを共有するには、両方の端末で同じメール
-- アドレス+パスワードでログインする必要があります(端末ごとに別ユーザーに
-- なってしまう匿名認証は、このアプリでは使用していません)。
-- 開発中や身内利用でメール確認の手間を省きたい場合は、Authentication > Providers >
-- Email の "Confirm email" をオフにすると、サインアップ直後から即ログイン状態になります
-- (本番でメール到達性を担保したい場合はオンのままにしてください)。
--
